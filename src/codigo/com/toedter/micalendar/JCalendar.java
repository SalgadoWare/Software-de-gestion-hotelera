...
import codigo.com.toedter.components.UTF8ResourceBundle;
...
public class JCalendar extends JPanel implements PropertyChangeListener {
	...
	public ArrayList<AnadirHorarioFrame> horarios = new ArrayList<>();
	...
	//	if (weekSelected != -1) {
	//		JOptionPane.showMessageDialog(new JFrame(), "Ya hay una semana seleccionada");
		//} else {
			//Date d = getDate();
			//Calendar c = new GregorianCalendar();
			//c.setTime(d);
			//weekSelected = c.get(Calendar.WEEK_OF_YEAR);
		//JOptionPane.showMessageDialog(new JFrame(), "Has seleccionado la semana " + weekSelected);
		//}
	//}
	...
	public void clean() {
		...
		horarios.clear();
		for (int i = 0; i < 7; i++)
			horarios.add(new AnadirHorarioFrame(this));
	}

	public void addHorarioFrame() {
		Date d = getDate();
		Calendar c = new GregorianCalendar();
		c.setTime(d);

		if (c.get(Calendar.WEEK_OF_YEAR) != weekSelected) {
			JOptionPane.showMessageDialog(new JFrame(), "Este dia no coresponde a la semana seleccionada");
		} else {

			int id = c.get(Calendar.DAY_OF_WEEK);
			switch (id) {
			case 2:

				horarios.get(0).setDate(d);
				horarios.get(0).setVisible(true);

				break;

			case 1:

				horarios.get(6).setDate(d);
				horarios.get(6).setVisible(true);

				break;

			default:

				horarios.get(id - 2).setDate(d);
				horarios.get(id - 2).setVisible(true);
	...
	public int updateHorasTotales() {
		int i = 0;

		for (int j = 0; j < horarios.size(); j++)
			i += horarios.get(j).horasTotales;

		return i;
	}

	public TransferHorario transferir() {

		if (updateHorasTotales() > 40) {
			JOptionPane.showMessageDialog(new JFrame(), "Has seleccionado mas de 40 horas");
			return null;
		} else {

			ArrayList<Dia> dias = new ArrayList<>();
			int i = 0;
			for (AnadirHorarioFrame a : horarios) {
				if (a.horasTotales != -1 && a.d != null) {

					Dia d = new Dia(a.firstHour, a.lastHour, a.first2Hour, a.last2Hour, a.d);
					dias.add(d);
					i = i + 1;
				}
			...
			if (i == 0) {
				JOptionPane.showMessageDialog(new JFrame(), "No has rellenado los dias");
				return null;
			} else {
				TransferHorario t = new TransferHorario(dias, uo, tipo);
	 ...
	 */
	public JCalendar(JLabel jLabel) {
		this(null, null, true, true,jLabel);
		for (int i = 0; i < 7; i++)
			horarios.add(new AnadirHorarioFrame(this));
	}
	 ...
	 */
	public JCalendar(Date date, Locale locale, boolean monthSpinner, boolean weekOfYearVisible, JLabel jLabel2) {
		...
		dayChooser = new JDayChooser(weekOfYearVisible, jLabel2);
...
//
//		JCalendar jcalendar = new JCalendar(null);
